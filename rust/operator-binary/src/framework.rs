...
///
/// Useful if you just want to reference an object
pub trait HasName {
    #[allow(dead_code)]
...
pub const fn min(x: usize, y: usize) -> usize {
    let one_if_x_lt_y = (x < y) as usize;
    one_if_x_lt_y * x + (1 - one_if_x_lt_y) * y
}
...
attributed_string_type! {
    ...
    // are valid, max_length is restricted. Compile-time check ensures that max_length cannot be
...
attributed_string_type! {
    ...
    // valid, max_length is restricted. Compile-time check ensures that max_length cannot be set
