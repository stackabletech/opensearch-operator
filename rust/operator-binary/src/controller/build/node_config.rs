...
impl NodeConfig {
    ...
    pub fn static_opensearch_config(&self) -> serde_json::Map<String, Value> {
        ...
        config.insert(
            ...
            json!(self.cluster.security_config.is_only_managed_by_api()),
        );
        if self.cluster.security_config.enabled
        ...
        config.insert(
            ...
            json!(!self.cluster.security_config.enabled),
        );
        config.insert
    ...
    pub fn admin_dn(&self) -> Option<String> {
        ...
        {
            Some(format!(
                "CN={container}.{pod}.{namespace}.{cluster_domain_name}",
                container = "update-security-config",
                pod = self.cluster.security_config.managing_role_group,
