...
The node role `coordinating_only` cannot be combined with other roles.
`nodeRoles: []` also defines a `coordinating_only` node.

[WARNING]
====
Do not remove the `data` node role from an existing role group!
Otherwise you have to repurpose the node to another role manually.
====
